...
def create_lookup_tables(text):
    ...
    #int_to_vocab = {id: word for id, word in vocab_to_int.items()}
...
def get_batches(int_text, batch_size, seq_length):
    ...
    # TODO: Implement Function
    # 这个接口有点难写
    # n_batches为例子中的First Batch, Second Batch
    ...
    # 这里不太理解，这一步参考的网上，但是不是很清楚为什么要加上inputs[0]，麻烦老师帮忙解释下
    targets = np.concatenate([ targets[:-1], [inputs[0]] ]) 
    
    x_batches = np.split(inputs.reshape(batch_size, -1), n_batch, 1)
    y_batches = np.split(targets.reshape(batch_size, -1), n_batch, 1)
    ret = np.array(list(zip(x_batches, y_batches)))
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    predicted_word = np.random.choice(list(int_to_vocab.values()),replace=False,p=probabilities)
